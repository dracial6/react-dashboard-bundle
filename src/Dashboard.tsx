...
class Dashboard extends React.Component {
  ...
  private alignContent = () => {
    ...
    for (const tag of arr) {
      ...
      if (this.containsBounds(priorityBounds.left, priorityBounds.top, priorityBounds.width, priorityBounds.height, tagBounds.left, tagBounds.top, tagBounds.width, tagBounds.height)) {
        if (tag !== this._currentMovingTag) { // 없으면 이동 중 정렬시 오류 발생
          ...
          if (parseInt(column) >= priorityColumn + priorityColumnCount) continue;
    ...
    for (const tag of overedTags) {
      ...
      (tag as HTMLElement).style.top = this.getRowY(priorityRow + priorityRowCount) + 'px';
      this.alignContent();
  ...
  private containsBounds = (targetX: number, targetY: number, targetWidth: number, targetHeight: number, x: number, y: number, width: number, height: number) => {
  ...
  private onTagMouseEnter = (e: any) => {
    if (this.state.isEditMode) {
      ...
      if (target) {
        (target.children[1] as HTMLElement).style.visibility = 'visible';
  ...
  private onTagMouseLeave = (e: any) => {
    if (this._currentOpenedTag) {
      (this._currentOpenedTag.children[1] as HTMLElement).style.visibility = 'hidden';
  ...
  registerSideMenu(menuID: string, title: string, description: string, imgSrc: string, minRowCount: number, minColumnCount: number, callBack: (e: SideMenuItem) => {}): void {
    ...
    this._sideMenuPanels.push(<SideMenuPanel id={menuID} title={title} description={description} imgSrc={imgSrc} onClick={this.onSideMenuPanelClick.bind(this)} minRowCount={minRowCount} minColumnCount={minColumnCount} style={{position: 'absolute', width: 'inherit', height: '160px', top: `${160 * this._sideMenuPanels.length / 2}px`, cursor: 'pointer', border: '1', borderColor: 'rgba(0,0,0,0.5)'}}></SideMenuPanel>);
    this._sideMenuPanels.push(<hr style={{position: 'absolute', top: `${160 * (this._sideMenuPanels.length + 1) / 2}px`, width: 'inherit', zIndex: 3, marginTop:'0'}}></hr>);
    this.forceUpdate();
  }
  ...
  render() {
    return (
      <div id={Constants.ID_Main} className={Constants.Class_Header} onLoad={this.onDashBoardLoad.bind(this)}>
        <div id={Constants.ID_Background} style={{visibility: 'hidden'}}></div>
          {this._components.map((value, key) => {
            return React.cloneElement(value, {key: key});
          })}
          <span>
            <img id={Constants.ID_Edit} src={offEdit} className='App-edit' alt='edit' width='100px' height='100px' onClick={this.onEditClick.bind(this)} onMouseEnter={this.onEditEnter.bind(this)} onMouseLeave={this.onEditLeave.bind(this)}
              style={{
                position: 'fixed',
          ...
          <div id={Constants.ID_SideMenu} style={{position: 'fixed', transition: 'left 500ms ease-out 0s', left: '100vw', zIndex: 4, visibility: 'hidden'}}>
            <img src={leftArrow} alt='leftArrow' width='40px' height='60px' onClick={this.onArrowClick.bind(this)} style={{ cursor: 'pointer', position: 'absolute', top: '45vh', left: '-40px' }}/>
            <div style={{position: 'absolute', width: `${this.state.sideMenuWidth}vw`, height: '100vh', backgroundColor: 'white', boxShadow: '0px 0px 10px 0px rgba(0,0,0,0.5)'}}>
              <h2 style={{position: 'absolute', left: '5px', top: '-15px'}}>Search</h2>
              <input id={Constants.ID_SideMenuSearch} type='text' onInput={this.onSearchInput.bind(this)} style={{position: 'absolute', left: '5px', top: '40px', width: '95%'}}></input>
              <div style={{position: 'absolute', top: '70px', width: 'inherit'}}>
                <hr style={{position: 'absolute', width: 'inherit', zIndex: 3, marginTop:'0'}}></hr>
                {this._sideMenuPanels.map((value, key) => {
                  return React.cloneElement(value, {key: key});
                })}
              </div>
              <div id={Constants.ID_SideMenuButtons} style={{position: 'absolute', transition: 'top 200ms ease-out 0s', width: 'inherit', height: `${this.state.sideMenuButtonsHeight}px`, top: '100%', boxShadow: '0px 0px 10px 0px rgba(0,0,0,0.5)'}}>
                <h2 style={{position: 'absolute', left: '5px', top: '-15px'}}>Info</h2>
                <p style={{position: 'absolute', left: '5px', top: '30px'}}>Title: </p><p style={{position: 'absolute', left: '45px', top: '30px'}}>{this.state.title}</p>
                <p style={{position: 'absolute', left: '5px', top: '55px'}}>Minimum Row Count: </p><p style={{position: 'absolute', left: '170px', top: '55px'}}>{this.state.minRowCount}</p>
                <p style={{position: 'absolute', left: '5px', top: '80px'}}>Minimum Column Count: </p><p style={{position: 'absolute', left: '195px', top: '80px'}}>{this.state.minColumnCount}</p>
                <p style={{position: 'absolute', fontSize: '15px', right:'60px', top:'-5px' }}>Row Count</p><input id={Constants.ID_SideMenuRowCount} style={{position: 'absolute', width:'110px', right:'20px', top:'35px'}} type='number' min='1'></input>
                <p style={{position: 'absolute', fontSize: '15px', right:'35px', top:'50px'}}>Column Count</p><input id={Constants.ID_SideMenuColumnCount} style={{position: 'absolute', width:'110px', right:'20px', top:'90px'}} type='number' min='1'></input>
                <input type='button' className='btnAdd' onPointerUp={this.onAddSideMenuClick.bind(this)} value='Add' style={{position: 'absolute', right: '70px', top: '140px'}}></input>
                <input type='button' className='btnClose' onPointerUp={this.onCancelSideMenuClick.bind(this)} value='Cancel' style={{position: 'absolute', right: '-10px', top: '140px'}}></input>
              </div>
            </div>
          </div>
      </div>
